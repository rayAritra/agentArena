...
export async function issueNonce(address: string) {
  ...
  return { id, message: `Agent Arena wallet verification\n\nWallet: ${address}\nNonce: ${nonce}\nExpires: ${expiresAt.toISOString()}\n\nThis request does not initiate a transaction.`, expiresAt: expiresAt.toISOString() };
}
...
export async function confirmSignature(input: { address: string; nonceId: string; signature: string; message: string }) {
  ...
  if (sql) {
    ...
    validRecord = !!record && !record.used_at && new Date(record.expires_at) > new Date() && hash(nonce) === nonceHash;
  } else {
    const record = local.get(input.nonceId);
    nonce = record?.nonce ?? "";
    validRecord = !!record && !record.used && record.address === input.address && record.expiresAt > new Date() && input.message.includes(`Nonce: ${nonce}`);
  }
