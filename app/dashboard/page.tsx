...
export default async function Dashboard(){
  ...
  return <div className="container py-12"><div className="flex flex-wrap items-end justify-between gap-4"><div><span className="eyebrow">Owner console</span><h1 className="mt-4 text-5xl font-black tracking-[-.03em]">Dashboard</h1><p className="mt-2 text-sm text-muted">Signed in as {user.email}</p></div><div className="flex gap-3"><Link href="/register" className="btn btn-primary btn-sm">Add agent</Link><SignOutButton/></div></div>
    <ScrollReveal className="mt-10 grid gap-5">{agents.length?agents.map((agent)=><article key={agent.slug} className="card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-3"><h2 className="text-2xl font-black">{agent.name}</h2><span className={`badge ${agent.verified_at?"badge-positive":"badge-warning"}`}>{agent.verified_at?"Verified":"Awaiting wallet signature"}</span></div><p className="mt-2 font-mono text-xs text-muted">{agent.address}</p></div><Link href={`/agent/${agent.slug}`} className="btn btn-outline btn-sm">Public profile</Link></div><div className="card mt-5 overflow-hidden"><MetricStrip className="grid-cols-2 md:grid-cols-4" items={[{label:"Portfolio",value:agent.portfolio_value_usd?`$${Number(agent.portfolio_value_usd).toLocaleString()}`:"—"},{label:"ROI",value:agent.roi?`${Number(agent.roi).toFixed(2)}%`:"—"},{label:"Trades",value:agent.trade_count??"0"},{label:"Sync block",value:agent.last_synced_block??"Not synced"}]}/></div><details className="mt-5"><summary className="cursor-pointer text-xs font-black text-brand">Edit agent</summary><EditAgentForm agent={agent}/></details></article>):<div className="card p-10 text-center"><p className="text-muted">No agents registered yet.</p><Link href="/register" className="btn btn-primary mt-5 inline-flex">Register your first agent</Link></div>}</ScrollReveal>
  </div>;
}
